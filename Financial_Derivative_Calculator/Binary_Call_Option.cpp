...
double Binary_Call_Option(double S,double K, double sigma, double r, double delta, double t, int step){
    ...
    double call_price [step+1]; //Binary Call option price array
    for (int i = 0; i <=step; i++) {
...
double Binary_Call_Option_continuous(double S,double K, double sigma, double r, double delta, double t){
    double d1;
    double d2;
...
int main()
{
    cout << Binary_Call_Option(40,40,0.3,0.08, 0.00, 1, 200)<< endl;
    //S,K,sigma,r,delta,T-t,total time steps
    cout << Binary_Call_Option_continuous(40,40,0.3,0.08, 0.00, 1);
	return 0;
}
